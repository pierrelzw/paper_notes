...
- 在使用AI进行辅助诊断时，医生希望AI告知结果时同时告诉他，它对这个结果有多确定，这样他就可以只关注AI不是很确定结果，从而减少工作量。

- **主动学习(Active Learning)中，哪些数据应该被标注？**(high uncertainty data)
...
基于目前的调研，研究深度学习(DL) uncertainty就是研究Bayesian Neural Network(BNN)。根据最近2周(0719-0731)的调研，学界、业界对**DL尤其是CV uncertainty**的研究比较少。
...
总结：uncertainty estimation是深度学习在实际使用时非常重要的一环。因为我们不仅希望AI输出预测结果，还想知道AI对结果的确定度，综合两者才能更好地使用DL模型。
...
为了建模计算Uncertainty，我们介绍了Monte-Carlo和Ensemble方法来建模Epistemic uncertainty，也介绍了Probabilistic DeepLearing用于计算(预测)Aleatoric Uncertainty。更多的建模方法，可参考对应图标中的参考文献。
